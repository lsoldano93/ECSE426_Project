...
void Master_Write(uint16_t VariableToWrite){
...
void Master_Communication(uint16_t LED_STATE, float* returnArray){
...
void DopeComs_Config(void){
	...
	NUCLEO_HS_CLOCK_ENABLE();
	NUCLEO_DATAi_CLOCK_ENABLE();                              
	
	// NUCLEO Output Pin (Output - Active High)
	GPIO_InitStructure.Pull  = GPIO_PULLDOWN;
	GPIO_InitStructure.Pin   = NUCLEO_DATAo0_PIN | NUCLEO_DATAo1_PIN | NUCLEO_DATAo2_PIN | NUCLEO_DATAo3_PIN;
	GPIO_InitStructure.Mode  = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_MEDIUM;
	HAL_GPIO_Init(NUCLEO_DATAo0_GPIO_PORT, &GPIO_InitStructure);
	
		
	// NUCLEO to Discovery Handshake Pin  (Output - Active High)
	GPIO_InitStructure.Pull  = GPIO_PULLDOWN;
	...
	// NUCLEO Input Pins (Input - Active High)
	GPIO_InitStructure.Pull  = GPIO_NOPULL;
	GPIO_InitStructure.Pin   = NUCLEO_DATAi0_PIN | NUCLEO_DATAi1_PIN | NUCLEO_DATAi2_PIN | NUCLEO_DATAi3_PIN;
